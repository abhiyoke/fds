...
int d[c1+1][3];            //matrix of second sparex matrix
d[0][0]=n;
d[0][1]=m;
d[0][2]=c1;
int k1=1;
...
cout<<"1st sparse matrix is:"<<endl;
for(int i=0;i<c+1;i++){
for(int j=0;j<3;j++){
cout<<b[i][j]<<" ";
}
...
cout<<"2nd sparse matrix is:"<<endl;
...
int s[c+c1+1][3];             //addition will contain both sparex matrix non-zero elements
int i=1,j=1,r=1;
int c2=0;
while(i<=c && j<=c1){
